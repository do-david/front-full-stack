...
export interface IAccountProps{
    userId:string;
    expiredSession:number;
}

const Account: FC<IUser>= () => {
    const [userDecoded] = useContext(AppContext)
    const session:IAccountProps = {
        userId:userDecoded.id,
        expiredSession:userDecoded.exp
    }
    const { loading, error, data } = useQuery(GET_USER, { variables: {id:session.userId}})
    return(
        <>
